...
type ManagerStaffPanelProps = {
  ...
  updateRecord: (id: string, payload: Partial<EmployeeCreate>) => Promise<Employee>;
...
function toEmployeePayload(values: EmployeeFormInput, fixedRole?: EmployeeCreate["roleKind"]): EmployeeCreate {
...
export function ManagerStaffPanel({
  ...
  const form = useForm<EmployeeFormInput>({
    ...
    defaultValues: { ...blankEmployee, roleKind: fixedRole ?? blankEmployee.roleKind },
    mode: "onBlur",
  });
  ...
  function openCreate() {
    setEditing(null);
    form.reset({ ...blankEmployee, roleKind: fixedRole ?? blankEmployee.roleKind });
    setIsOpen(true);
  }

  function openEdit(employee: Employee) {
    setEditing(employee);
    form.reset({ ...employeeToForm(employee), roleKind: fixedRole ?? employee.roleKind });
    setIsOpen(true);
  }
  ...
  async function deactivate(employee: Employee) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : `Unable to deactivate ${title.toLowerCase()}.`);
    }
  }
  ...
  const columns: ColumnDef<Employee>[] = [
    ...
    },
    {
      ...
      header: "Role",
      cell: ({ row }) => <span className="capitalize">{roleLabel(row.original.roleKind)}</span>,
    },
    {
  ...
  return (
    <Card className="rounded-md border-zinc-200 shadow-none">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
              <DialogHeader>
                <DialogTitle>{editing ? `Edit ${title.toLowerCase()}` : createLabel}</DialogTitle>
                <DialogDescription>Changes are saved to the current organization.</DialogDescription>
              </DialogHeader>
              <form id="manager-staff-form" className="space-y-5" onSubmit={form.handleSubmit(saveEmployee)} noValidate>
                <EmployeeFormFields control={form.control} fixedRole={fixedRole} />
      ...
      <CardContent className="space-y-4">
        ...
        <DataTable columns={columns} data={activeEmployees} emptyLabel={isLoading ? "Loading..." : emptyLabel} />
      </CardContent>
    </Card>
  );
}
